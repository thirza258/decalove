...
| (not addressed) | **Character sprites are keyed and seeded on the character alone** | A generated image is only as stable as the request behind it. Sprites are background-removed and composited over separately generated scenery, so scene context in a sprite prompt is discarded *after* re-lighting and re-framing the character — and keying on it made ~670 independently generated sprites where 28 would do. Each character's images now share one derived seed, so every expression starts from the same face. See `api/app/agents/visual.py`. |
