...
import { MARK_COLORS, newBlock } from "../writing/model";
import type { BlockKind, MarkColor, MarkKind, PictureTarget, WritingBlock, WritingDocument } from "../writing/model";
...
const alignImage = (alignment: WritingBlock["alignment"]) =>
  alignment === "center" ? { marginInline: "auto" } : alignment === "right" ? { marginLeft: "auto" } : { marginRight: "auto" };

export function WritingEditor({ doc, selectedId, onSelect, onEdit, onPickImage, onComment, undo, redo, canUndo, canRedo }: Props) {
  const workspace = useWritingWorkspace();
  const [finding, setFinding] = useState(false);
  const [focusMode, setFocusMode] = useState(false);
  const [palette, setPalette] = useState<"" | "color" | "highlight">("");
  ...
  for (const comment of doc.comments) {
    if (!comment.resolved && comment.blockId) openComments.set(comment.blockId, (openComments.get(comment.blockId) ?? 0) + 1);
  }

  function updateBlock(id: string, patch: Partial<WritingBlock>, group = "") {
    onEdit({ blocks: doc.blocks.map((b) => b.id === id ? { ...b, ...patch } : b) }, group);
  }
  function insert(block: WritingBlock) {
  ...
  /** With words selected the range is formatted; with none, the whole passage is. */
  function format(kind: MarkKind, value?: MarkColor) {
    if (!selected) return;
    ...
    updateBlock(selected.id, { marks: toggleMark(selected.marks ?? [], start, end, kind, value) });
  }
  function find() {
    setFinding(true);
  ...
  function colorMenu(kind: "color" | "highlight", label: string, glyph: string) {
    return (
      <div className="color-menu">
        <button aria-label={label} title={`${label} — select words or a whole passage`} aria-expanded={palette === kind}
          disabled={!selected?.text} onMouseDown={keepSelection} onClick={() => setPalette(palette === kind ? "" : kind)}>
          {glyph}<span aria-hidden="true" className="menu-caret">▾</span>
        </button>
        {palette === kind && <div className="swatches" role="group" aria-label={`${label} palette`}>
          {MARK_COLORS.map((color) => (
            <button key={color.id} className={`swatch mark-${kind === "highlight" ? "highlight" : "color"}-${color.id}`}
              aria-label={`${label}: ${color.label}`} title={color.label}
              onMouseDown={keepSelection} onClick={() => { format(kind, color.id); setPalette(""); }}>A</button>
          ))}
          <button className="swatch none" aria-label={`${label}: none`} title="None"
            onMouseDown={keepSelection} onClick={() => { format(kind); setPalette(""); }}>⌀</button>
        </div>}
      </div>
    );
  }
  ...
  return (
    <section className={`writing-desk ${focusMode ? "focus-mode" : ""}`} aria-label="Document editor" onKeyDown={(event) => {
      ...
      <div className="editor-toolbar" role="toolbar" aria-label="Document tools">
        ...
        <button disabled={doc.blocks.length >= 1000} title="Insert a picture after the selected passage"
          onClick={() => onPickImage("Insert a picture", { kind: "passage" })}>+ Image</button>
        <span className="toolbar-divider" />
        <button disabled={!selected} title="Comment on the selected passage" onClick={() => selected && onComment(selected.id)}>💬 Comment</button>
      ...
      <div className="formatting-toolbar" role="toolbar" aria-label="Text formatting">
        ...
        {([['bold', 'B'], ['italic', 'I'], ['underline', 'U']] as const).map(([kind, label]) => <button className={`format-${kind}`} key={kind} title={`${kind} (Ctrl/⌘+${label}) — select words or a whole passage`} aria-label={kind[0].toUpperCase() + kind.slice(1)} disabled={!selected?.text} onMouseDown={keepSelection} onClick={() => format(kind)}>{label}</button>)}
        {colorMenu("color", "Text colour", "A")}
      ...
      <div className="paper-area">
        <div className={`manuscript ${doc.format}`} style={{ zoom: zoom / 100 }}>
          ...
          {doc.blocks.every((b) => !b.text.trim() && b.kind !== "image") && <p className="editor-hint">Start with a place, a voice, or something left unsaid.<br />Write below, or build a scene with your AI writing partner.</p>}
          <div className="manuscript-blocks">
            {doc.blocks.map((block, index) => (
              <div key={block.id} className={`manuscript-block ${block.kind} list-${block.listStyle ?? "none"} ${selectedId === block.id ? "selected" : ""}`}
                onFocus={() => onSelect(block.id)} onClick={() => block.kind === "image" && onSelect(block.id)}>
                <span className="block-number" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
                {!!openComments.get(block.id) && <button className="comment-marker" onClick={() => onComment(block.id)}
                  aria-label={`${openComments.get(block.id)} open comment${openComments.get(block.id) === 1 ? "" : "s"} on passage ${index + 1}`}>💬 {openComments.get(block.id)}</button>}
                ...
                {block.kind === "image" && block.image && <figure className="passage-image" style={{ width: `${block.image.width}%`, ...alignImage(block.alignment) }}>
                  <img src={workspace.imageUrl(block.image.id)} alt={block.image.alt} loading="lazy" />
                </figure>}
                <RichPassage block={block} label={`${block.kind === "dialogue" ? "Dialogue" : block.kind === "heading" ? "Heading" : block.kind === "image" ? "Caption" : "Narration"} passage ${index + 1}`}
                  placeholder={block.kind === "dialogue" ? "What do they say?" : block.kind === "heading" ? "A chapter or scene title…" : block.kind === "image" ? "Caption this picture (optional)…" : "Set the scene. Let us notice something…"}
                  onChange={(patch) => updateBlock(block.id, patch, `text-${block.id}`)} />
                {block.kind === "image" && block.image && <div className="image-tools">
                  <label htmlFor={`alt-${block.id}`}>Description</label>
                  <input id={`alt-${block.id}`} aria-label={`Description of picture ${index + 1}`} value={block.image.alt} maxLength={300}
                    placeholder="What is in this picture?" onChange={(e) => updateBlock(block.id, { image: { ...block.image!, alt: e.target.value } }, `alt-${block.id}`)} />
                  <select aria-label={`Width of picture ${index + 1}`} value={block.image.width}
                    onChange={(e) => updateBlock(block.id, { image: { ...block.image!, width: Number(e.target.value) } })}>
                    {[25, 50, 75, 100].map((width) => <option key={width} value={width}>{width}%</option>)}
                  </select>
                  <button onClick={() => onPickImage("Replace this picture", { kind: "replace", id: block.id })}>Replace</button>
                </div>}
                <div className="block-controls">
                  {block.kind !== "image" && <select aria-label={`Type of passage ${index + 1}`} value={block.kind} onChange={(e) => updateBlock(block.id, { kind: e.target.value as BlockKind, speaker: e.target.value === "dialogue" ? block.speaker : "" })}>
      ...
      <article className="print-manuscript"><h1>{doc.title}</h1>{doc.blocks.filter((b) => b.text.trim() || b.kind === "image").map((b) => <section className={`list-${b.listStyle ?? "none"}`} key={b.id} style={{ textAlign: b.alignment, fontFamily: b.font ? FONTS[b.font] : undefined, fontSize: `${b.size ?? 16}px` }}>
        {b.kind === "dialogue" && <strong>{b.speaker}</strong>}
        {b.kind === "image" && b.image && <img src={workspace.imageUrl(b.image.id)} alt={b.image.alt} style={{ width: `${b.image.width}%` }} />}
