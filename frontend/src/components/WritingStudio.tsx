...
import { downloadFile, exportText, importText, isDocument, newBlock, newDocument, THEME_STARTERS } from "../writing/model";
import type { WritingBlock, WritingSuggestion } from "../writing/model";
...
export default function WritingStudio({ exercise, onExerciseUsed }: { exercise: string; onExerciseUsed: () => void }) {
  ...
  function insert(blocks: WritingSuggestion["blocks"], original: WritingBlock | null): string | null {
    ...
    if (original) {
      ...
      next = [...doc.blocks.filter((b) => b.text.trim() || b.speaker.trim()), ...fresh];
    }
  ...
  async function importFile(file: File) {
    ...
    try {
      ...
      if (file.name.toLowerCase().endsWith(".json")) {
        ...
        imported = { ...backup.document, id: crypto.randomUUID(), updatedAt: new Date().toISOString() };
      } else {
  ...
  return (
    <div className="writing-app studio-app" onKeyDown={(event) => {
      ...
      <header className="studio-heading">
        <div><p className="eyebrow">A space to make something yours</p><h1>Writing Studio<span>.</span></h1></div>
      ...
      <div className="mobile-studio-tabs" role="group" aria-label="Writing workspace panels">{([['brief', 'Story brief'], ['editor', 'Manuscript'], ['assistant', 'AI partner']] as const).map(([panel, title]) => <button key={panel} aria-pressed={mobilePanel === panel} onClick={() => setMobilePanel(panel)}>{title}</button>)}</div>
      <main className={`studio-layout show-${mobilePanel}`}>
        <aside className="story-panel" aria-label="Story brief">
          ...
          <div className="story-panel-note"><span aria-hidden="true">⌁</span><p>Drafts and lessons save to your workspace. Keep your <a href={`#/studio?workspace=${workspace.id}`}>private workspace link</a> to open it on another device. Backups include your story notes.</p></div>
        </aside>
        <WritingEditor key={`editor-${doc.id}`} doc={doc} selectedId={selectedId} onSelect={setSelectedId} onEdit={edit} undo={library.undo} redo={library.redo} canUndo={library.canUndo} canRedo={library.canRedo} />
        <WritingAssistant key={`assistant-${doc.id}`} doc={doc} selectedId={selectedId} onEdit={edit} onInsert={insert} />
      </main>
    </div>
  );
}
